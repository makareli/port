...
def add_ship_to_excel():
    try:
        while True:
            try_ship_num = int(input("enter ship's number (has to be six digits): "))
            if ship_num_exists(try_ship_num):
                if 000000 <= try_ship_num <= 999999:
                    ship_num = try_ship_num
                    break
        ...
        if cargo == '':
            cargo = 'No Cargo'
        if len(cargo) > 6 and cargo[-1] == 'x'.lower() and ship_flag.name().lower() == 'iran':
            while True:
        ...
        wb = load_workbook("Namal.xlsx")
        ws = wb.active
        new_row_data = [ship_num, ship_name.capitalize(), ship_flag.name().capitalize(), cargo,
                        date, "/", activity.capitalize()]
